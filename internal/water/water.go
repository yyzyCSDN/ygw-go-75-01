...
type Store struct {
	...
	pondLocks map[string]*sync.Mutex
...
func (s *Store) pondLock(pondID string) *sync.Mutex {
	s.mu.RLock()
	lock, ok := s.pondLocks[pondID]
	s.mu.RUnlock()
	if ok {
		return lock
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	lock = s.pondLocks[pondID]
	if lock == nil {
		lock = &sync.Mutex{}
		s.pondLocks[pondID] = lock
	}
	return lock
}

func (s *Store) ApplyMetrics(pondID string, patch MetricPatch) error {
	lock := s.pondLock(pondID)
	lock.Lock()
	defer lock.Unlock()
	st, ok := s.states[pondID]
	if !ok {
